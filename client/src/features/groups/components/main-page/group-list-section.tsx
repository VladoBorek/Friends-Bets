...
export function GroupListSection({ groups, totalGroups, currentPage, totalPages, selectedGroupId, isRefreshing, onGroupSelect, onPageChange }: GroupListSectionProps) {
  return (
    <section className="flex flex-col gap-4">
      ...
      {groups.length === 0 ? (
        <Card className="rounded-2xl border-slate-800 p-5 text-sm text-slate-400">No groups found.</Card>
      ) : (
        <div className="grid gap-5 lg:grid-cols-2 2xl:grid-cols-3">
          {groups.map((group) => (
            <GroupCard key={group.id} group={group} isSelected={group.id === selectedGroupId} onClick={() => onGroupSelect(group)} />
          ))}
        </div>
      )}
