...
import { buildPreviewRows, getMoneyTone } from "../../utils/group-display";
    ...
    <button type="button" onClick={onClick} className="group w-full text-left">
      <article
        className={cn(
          ...
          "transition-[transform] duration-200 ease-out motion-safe:group-hover:-translate-y-1 motion-safe:group-hover:scale-[1.015]",
        ...
        <div className="flex min-w-0 items-start gap-3 sm:gap-4">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex min-w-0 items-center gap-2">
              ...
              {isOwner ? <span className="shrink-0 rounded-md border border-amber-400/20 bg-amber-500/10 px-2 py-0.5 text-[0.68rem] font-semibold text-amber-300">OWNER</span> : null}
            </div>
        ...
        <div className="mt-4">
          ...
          <div className="mt-4 grid gap-3">
            <div className="flex items-center justify-between gap-4">
              ...
              <span className={cn("font-mono text-sm font-semibold", getMoneyTone(group.netPnl))}>{group.netPnl}</span>
            </div>
            ...
            {previewRows.map((member, index) => (
              <div key={member?.id ?? `empty-${index}`} className="flex h-5 items-center justify-between gap-4">
                {member ? (
                  <>
                    <span className="truncate text-sm font-semibold text-slate-100">{index + 1}. {member.username}</span>
