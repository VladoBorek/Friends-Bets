...
async function toSummary(
  ...
  const [group, requester, addressee, memberCount, activeWagerCount, members] =
    await Promise.all([
...
export async function sendGroupInvitation(
  ...
  const row =
    existingInvite?.status === "REJECTED"
      ...
      : await groupInvitationRepository.createGroupInvitation(
          groupId,
          currentUserId,
          input.addresseeId,
        );
